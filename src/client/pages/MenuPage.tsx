// ABOUT: Main menu — side/difficulty selection and primary navigation.
// ABOUT: Shows anonymous games counter (read-only GET). No auth in v0.1.

import { useState, useEffect } from 'react';
import { Shield, Swords } from 'lucide-react';
import type { Side, Difficulty } from '@/shared/game/types';
import { Button } from '@/client/components/ui/button';
import { fetchAnonymousGamesCount } from '@/client/lib/api';
...
export default function MenuPage({ onStartGame, onShowRules, onShowPrivacy }: MenuPageProps) {
  ...
  useEffect(() => {
    fetchAnonymousGamesCount()
      .then(count => setGamesCount(count))
      .catch(() => { /* silently ignore — counter is cosmetic */ });
  }, []);
    ...
    <div className="min-h-screen bg-[#f5f0e8] flex flex-col items-center justify-center relative overflow-hidden px-4">
      ...
      <div className="relative z-10 max-w-lg w-full" style={{ fontFamily: 'Cinzel, serif' }}>
        ...
        <div className="text-center mb-10">
          ...
          {gamesCount !== null && (
            <p className="text-[#8b7a68] text-[10px] tracking-[0.25em] uppercase mt-1">
              {gamesCount.toLocaleString()} {gamesCount === 1 ? 'game' : 'games'} played
            </p>
          )}
        </div>
