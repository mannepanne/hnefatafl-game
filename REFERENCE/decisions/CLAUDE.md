...
- [2026-05-12 — Send email through an Emailer interface — Cloudflare Email Sending primary, Resend fallback](./2026-05-12-email-provider-abstraction.md) — single `Emailer` interface with `CloudflareEmailer` (beta, primary) and `ResendEmailer` (from `hultberg.org`, fallback). Manual switchover via Wrangler secret + redeploy.
